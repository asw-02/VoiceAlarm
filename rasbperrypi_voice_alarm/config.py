...
class TTSConfig:
    """Configuration for offline text-to-speech via Piper."""
    BASE_DIR = Path(__file__).resolve().parent

    ENABLED = True
    PIPER_BINARY = "piper"
    PLAYBACK_BINARY = "aplay"

    MODEL_PATH = BASE_DIR / "models" / "de_DE-thorsten-medium.onnx"
    MODEL_CONFIG_PATH = BASE_DIR / "models" / "de_DE-thorsten-medium.onnx.json"
    CACHE_ENABLED = True
    CACHE_DIR = BASE_DIR / "audio_cache"
    PREWARM_CACHE_ON_START = True

    DEFAULT_VOICE_PROMPT = "Ich höre."
    CONFIRM_RETRY_PROMPT = "Bitte antworte mit ja oder nein."
    CONFIRM_TIMEOUT_PROMPT = "Bestätigung abgebrochen."
    STANDARD_CACHE_TEXTS = (
        DEFAULT_VOICE_PROMPT,
        CONFIRM_RETRY_PROMPT,
        CONFIRM_TIMEOUT_PROMPT,
        "Befehl nicht verstanden.",
        "Ich habe nichts verstanden.",
        "Bitte nenne eine Uhrzeit.",
        "Es gibt keine Wecker zum Loeschen.",
        "Es gibt keinen aktiven Wecker.",
        "Diesen Wecker konnte ich nicht finden.",
        "Der angeforderte Wecker wurde nicht gefunden.",
        "In Ordnung, abgebrochen.",
        "In Ordnung. Ich habe alle Wecker geloescht.",
